...
func (q *Queries) TransferMoney(ctx context.Context, arg TransferMoneyParams) (Account, error) {
	row := q.db.QueryRowContext(ctx, transferMoney, arg.Amount, arg.ID)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
...
func (q *Queries) UpdateBalance(ctx context.Context, arg UpdateBalanceParams) error {
	_, err := q.db.ExecContext(ctx, updateBalance, arg.ID, arg.Balance)
	return err
}
